...
type Config struct {
	...
	// elections（选举）. That is, if a follower does not receive（接收） any message from the
	// leader of current term before ElectionTick has elapsed（过去）, it will become
	// candidate（候选人） and start an election. ElectionTick must be greater than
	// HeartbeatTick. We suggest ElectionTick = 10 * HeartbeatTick to avoid（避免）
	// unnecessary（不必要） leader switching（切换）.
	ElectionTick int
	// HeartbeatTick is the number of Node.Tick invocations that must pass between
	// heartbeats. That is, a leader sends heartbeat messages to maintain（保持） its
	// leadership every HeartbeatTick ticks.
	HeartbeatTick int

	// Storage is the storage for raft. raft generates entries（条目） and states to be
	// stored in storage. raft reads the persisted（持久化）entries and states out of
	// Storage when it needs. raft reads out the previous（之前）state and configuration
	// out of storage when restarting.
	Storage Storage
	// Applied is the last applied（应用） index. It should only be set when restarting
	...
	// value lowers（降低） the raft recovery（恢复） cost(initial probing（探测） and message lost
	// during（在期间） normal operation). On the other side, it might affect（影响） the
	// throughput（吞吐） during normal replication（复制）. Note: math.MaxUint64 for unlimited,
	// 0 for at most one entry per（每一条） message.
	MaxSizePerMsg uint64
	// MaxCommittedSizePerReady limits the size of the committed entries which
	// can be applied.
	MaxCommittedSizePerReady uint64
	// MaxUncommittedEntriesSize limits the aggregate byte（聚合字节） size of the
	// uncommitted entries that may be appended to a leader's log. Once this
	// limit is exceeded（超过）, proposals（提案） will begin to return ErrProposalDropped
	// errors. Note: 0 for no limit.
	MaxUncommittedEntriesSize uint64
	// MaxInflightMsgs limits the max number of in-flight append messages during
	// optimistic replication phase（阶段）. The application transportation layer usually
...
func (r *raft) Step(m pb.Message) error {
	...
	case m.Term == 0:
		// local message
	case m.Term > r.Term:
		// 消息的任期大于本地的任期，判断消息类型是否为选举
		if m.Type == pb.MsgVote || m.Type == pb.MsgPreVote {
			// leader 转移标记
			force := bytes.Equal(m.Context, []byte(campaignTransfer))
			// 不是主节点且在超时的时间内收到主节点发来消息，就不投票了
